use crate::block::originator::genesis as originator_genesis;
use crate::block::picker::genesis as picker_genesis;
...
impl BlockDatabase {
    ...
    pub fn new<P: AsRef<std::path::Path>>(
        ...
        // insert originator genesis block
        db.db.put_cf(
            block_cf,
            &config.originator_genesis,
            &serialize(&originator_genesis()).unwrap(),
        )?;
        db.db.put_cf(
        ...
        // insert picker genesis blocks
        for i in 0..config.picker_chains {
            db.db.put_cf(
                block_cf,
                &config.picker_genesis[i as usize],
                &serialize(&picker_genesis(i as u16)).unwrap(),
            )?;
            db.db.put_cf(
                block_arrival_order_cf,
                &counter.to_ne_bytes(),
...
mod tests {
    ...
    fn insert_contains_and_get() {
        ...
        assert_eq!(num_block, 1 + NUM_PICKER_CHAINS as u64 + 1);
    ...
    fn blocks_after() {
        ...
        .unwrap();
        // try to get all blocks after the originator genesis
        let iter = db.blocks_after(&(*ORIGINATOR_GENESIS_HASH), 2);
        let mut next_picker = 0;
        for batch in iter {
            if next_picker + 1 < NUM_PICKER_CHAINS {
                assert_eq!(batch[0].hash(), picker_genesis(next_picker).hash());
                assert_eq!(batch[1].hash(), picker_genesis(next_picker + 1).hash());
                next_picker += 2;
            } else {
                assert_eq!(batch[0].hash(), picker_genesis(next_picker).hash());
                next_picker += 1;
        ...
        assert_eq!(next_picker as u16, NUM_PICKER_CHAINS as u16);
    }
    ...
    fn latest_block_hash() {
        ...
        assert_eq!(
            ...
            PICKER_GENESIS_HASHES[NUM_PICKER_CHAINS as usize - 1]
        );
    }
}
*/
